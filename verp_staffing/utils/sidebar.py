...
def get_sidebar_permissions():
	...
	frappe.cache().set_value(_cache_key(user), payload, expires_in_sec=CACHE_TTL_SECONDS)
	return payload
...
def _accessible(item: dict, is_admin: bool, user_roles: set, link_field: str) -> bool:
	...
	if item_kind == "report":
		...
		if not report_roles:
			# No roles configured on the Report itself → not restricted
			return True

		return bool(user_roles.intersection(report_roles))

	return True

def _get_config_for_user(user: str):
	config_json = frappe.db.get_value("Sidebar Master", {"sidebar_owner": user}, "config_json")
	if not config_json:
	...
	try:
		...
		frappe.log_error(
			title="verp_staffing sidebar: invalid config_json", message=frappe.get_traceback()
		)
		return []
...
def _sync_designation_roles_from_employee(doc, method=None):
	...
	if to_remove:
		user_doc.set(
			...
			[
				d
				for d in user_doc.roles
				if d.role not in to_remove
			]
		)
...
def on_has_role_change(doc, method=None):
	...
	if method == "on_trash" and doc.role in MANDATORY_ROLES_WHILE_ACTIVE:
		employee = frappe.db.get_value("Employee", {"user": doc.parent, "enabled": 1}, "name")
		if employee:
			frappe.get_doc(
				{
					"doctype": "Has Role",
