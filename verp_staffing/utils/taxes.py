...
import verp_staffing
from verp_staffing.utils.accounts import get_exchange_rate as _get_exchange_rate
from verp_staffing.utils.accounts import get_company_currency
...
@frappe.whitelist()
def calculate_taxes_and_totals_function(doc):
    import json
    ...
    doc.company_currency = doc.company_currency or frappe.get_cached_value(
        "Company", doc.company, "default_currency"
    )

    doc.currency = doc.currency or doc.company_currency

    doc.conversion_rate = doc.conversion_rate or 1

    # 🔥 THIS IS THE CRITICAL FIX LINE
    doc = prepare_tax_doc(doc)
...
def prepare_tax_doc(doc):
    # 1. ensure taxes exist
    doc.taxes = doc.get("taxes") or []

    # 2. enforce order (VERY IMPORTANT)
    doc.taxes = sorted(doc.taxes, key=lambda x: x.get("idx", 0))

    # 3. rebuild idx properly
    for i, tax in enumerate(doc.taxes, start=1):
        tax.idx = i

    # 4. fix row_id dependencies (THIS FIXES YOUR ERROR)
    for i, tax in enumerate(doc.taxes, start=1):

        if tax.charge_type in ["On Previous Row Amount", "On Previous Row Total"]:

            # invalid or missing row_id → fix it
            if not tax.get("row_id") or cint(tax.row_id) >= i:
                tax.row_id = i - 1 if i > 1 else None
...
class calculate_taxes_and_totals:
	...
	def set_rounded_total(self):
		...
		if self.doc.meta.get_field("rounded_total"):
			if is_rounded_total_disabled(self.doc):
