...
frappe.ui.form.on("Resume", {
    ...
    validate(frm) {
        if (
            ...
            frappe.msgprint(
                "Status will be set to Completed automatically when you upload a resume PDF."
            );
            frappe.validated = false;
        }

        self._validate_resume_format()
    ...
    resume(frm) {
        if (frm.doc.resume) {
            ...
            frappe.show_alert({
                message: __("Resume uploaded. Status marked as Completed."),
                indicator: "green"
            });
        }
    },
