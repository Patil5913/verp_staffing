...
class Employee(Document):    
    ...
    def validate(self):
        ...
        rows = (
            self.employee_assignment_details_table
            or []
        )
    ...
    def _validate_assignment_rows(self, rows):
        ...
        for row in rows:
            ...
            child_roles = child_roles_cache.get(
                department
            )
            ...
            if child_roles is None:
                ...
                hierarchy_json = hierarchy_map.get(
                    department
                )
...
def get_users_not_linked_to_employee(
    ...
    if txt:
        conditions.append(
            f"(u.name LIKE %s OR u.full_name LIKE %s)"
        )
...
def get_employee_from_user(user):
    ...
    cache_key = (
        f"employee_from_user::{user}"
    )

    employee = frappe.cache().get_value(
        cache_key
    )
...
def get_user_departments(user=None):
    ...
    cache_key = (
        f"user_departments::{employee}"
    )

    departments = frappe.cache().get_value(
        cache_key
    )
