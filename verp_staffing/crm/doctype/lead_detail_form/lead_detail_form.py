...
class LeadDetailForm(Document):
    ...
    def before_save(self):
        ...
        # Ensure form visit logs exists
        form_logs = audit_from_js.get("form visit logs", [])

        # Get server-trusted IP + UA
        signer_location = get_ip_and_device()

        # Append SINGLE server-truth submission event
        form_logs.append({
            "event": "form_submitted",
            "timestamp": format_timestamp_utc(),
            "ip": signer_location.get("ip"),
            "user_agent": signer_location.get("user_agent"),
        })

        audit_from_js["form visit logs"] = form_logs

        self.audit_trail = json.dumps(audit_from_js, indent=2)
...
#     # ---------------------------------------------------------
#     # CERTIFICATE ID (GENERATE ONCE)
#     # ---------------------------------------------------------
#     if agreement and not agreement.certificate_id:
#         certificate_id = generate_certificate_id(agreement.name)
#         agreement.db_set("certificate_id", certificate_id)
#     else:
#         certificate_id = agreement.certificate_id if agreement else None
...
#     # ---------------------------------------------------------
#     # STEP 4: HASH FINAL DOCUMENT (CRITICAL)
#     # ---------------------------------------------------------
