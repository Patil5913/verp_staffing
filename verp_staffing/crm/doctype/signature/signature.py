...
def generate_pdf_pages(docname):
    ...
    doc = frappe.get_doc("Signature", docname)
...
def format_timestamp_utc():
    # Get current time with UTC timezone awareness
    d = datetime.now(timezone.utc)
    
    # Use strftime to format the date and time string
...
def save_signature(field_name=None, image=None, token=None):
    ...
    agreement = frappe.get_doc("Signature", field.parent)
...
def send_signer_email(agreement, email):
    ...
    doc = frappe.get_doc("Signature", agreement)
...
def generate_certificate_page(agreement_name):
    ...
    for email, field in unique_signers.items():
        ...
        # -------------------------
        # Sign Activity PER SIGNER
        # -------------------------
        signer_activity = [a for a in agreement.sign_activity if a.email == email]
...
def track_ip_and_device(browser=None, os=None, device=None, token=None):
    ...
    agreement.append("Sign Activity", {
        "email": signer_email,
