    ...
    refresh(frm) {
        frappe.call({
            ...
            callback(r) {
                ...
                const servicesPlus = [...services, "Lead"];
...
const SERVICE_ICONS = {
    "marketing": "📣", "resume": "📄", "cover letter": "✉️",
    "ruc": "🗂️", "training": "🎓", "jdc": "📋",
    "background check": "🔍", "reference check": "👥",
    "drug test": "🧪", "employment verification": "🏢",
    "education check": "🎒", "credit check": "💳",
    "onboarding": "🚀", "interview": "💬", "assessment": "📝",
};
...
const WIDGET_INTENTS = {
    ...
    candidate_form: {
        ...
        description: "Controls <strong>which fields appear</strong> on the candidate's application form for a specific service. Each service shows only its relevant fields — no clutter, no confusion for the candidate.",
        steps: ["Select a service", "Configure service options", "Tick the fields to show", "Save"],
    ...
    department_display: {
        ...
        description: "Controls <strong>which fields are visible</strong> to a department when viewing a candidate's details form under a specific department. Sensitive data can be hidden from departments that don't need it.",
    ...
    department_access: {
        ...
        description: "Controls <strong>which fields a department can edit</strong> on a candidate's profile. Select a department to define edit authority for that context — all other fields become read-only.",
...
function render_field_selector_widget(frm, htmlFieldname, storageField, intentKey, itemsList) {
    ...
    const EXCLUDE_FIELDS = [
        ...
        "docstatus", "idx", "form_token", "signature_method", "signature",
        "agreement_html", "authentication_html", "signature_image",
        "signature_custom_html", "files_custom_html", "reference_table",
    ...
    function setFields(name, fields) {
        if (isCandidateForm) {
            if (!CONFIG[name]) CONFIG[name] = { fields: [], is_agreement_required: false, is_candidate_form_required: false };
    ...
    frappe.model.with_doctype("Lead Detail Form", function () {
        ...
        meta.fields
            ...
            .forEach(df => {
                ...
                if (["Column Break", "HTML", "Fold", "Heading", "Tab Break"].includes(df.fieldtype)) return;
        ...
        // ── Intent header ─────────────────────────────────────────────────
        const stepsHtml = intent.steps.map((s, i) => `
        ...
        grid.on("click", `.cfs-item-${uid}`, function () {
            ...
            $(this).css({ border: `1.5px solid ${intent.accentColor}`, background: intent.accentBg });
            $(this).find(`.cfs-item-ic-${uid}`).css({ background: intent.accentColor, color: "#fff" });
            currentItem = $(this).data("name");
            wrapper.find(`#${uid}-sn2`).css({ background: intent.accentColor, color: "#fff", border: `1px solid ${intent.accentColor}` });
        ...
        function renderPanel(q) {
            ...
            SECTIONS.forEach(sec => {
                const vis = sec.fields.filter(f =>
                    !lq || f.label.toLowerCase().includes(lq) || f.fieldname.toLowerCase().includes(lq)
                );
            ...
            wrapper.find(`.${uid}-cb`).on("change", function () {
                ...
                if (on) { if (!fields.includes(fn)) fields.push(fn); }
                else { fields = fields.filter(x => x !== fn); }
                setFields(currentItem, fields);
                $(this).closest("label").css("background", on ? intent.accentBg : "transparent");
        ...
        function syncCardCount() {
            ...
            wrapper.find(`.cfs-item-${uid}[data-name="${escapeQ(currentItem)}"] .cfs-item-ct-${uid}`)
                .text(`${cnt} field${cnt !== 1 ? "s" : ""}`);
        }

        function updateFooter() {
            const cnt = getFields(currentItem).length;
            wrapper.find(`#${uid}-fm`).html(
