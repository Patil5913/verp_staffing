...
frappe.ui.form.on("Hierarchy", {
	async refresh(frm) {
		...
		frm.set_query("department", () => {
			return {
				filters: [["Department", "name", "not in", frm.used_departments || []]],
			};
		});
	...
	async before_save(frm) {
		...
		if (!frm.doc.auto_assign_config) {
			frappe.msgprint(
				__(
					"Please select a role for Auto Assignment so the system knows who can receive records.",
				),
			);
			frappe.validated = false;
		}
...
async function load_department_roles(frm) {
	...
	if (res?.message?.role && Array.isArray(res.message.role)) {
		roles = res.message.role.map((row) => row.role).filter((r) => r);
	}

	if (!roles.length) {
		const department_link = frappe.utils.get_form_link("Department", frm.doc.department, true);

		frappe.msgprint({
			title: __("No Roles Configured"),
			indicator: "orange",
			message: `
            The selected department: ${department_link} does not have any roles assigned.
            <br><br>

            Please configure roles in the department: ${department_link} before defining hierarchy relationships.
            <br><br>
        `,
		});
...
function add_row(frm, rowData) {
	...
	function render_dropdown(filter = "") {
		...
		let roles = (frm.department_roles || []).filter(
			(r) => r.toLowerCase().includes(filter.toLowerCase()) && !selected_roles.includes(r),
		);
	...
	if (rowData?.child_roles?.length) {
		selected_roles = rowData.child_roles.map((r) => (typeof r === "string" ? r : r.role));
		render_chips();
	}
...
function render_hierarchy_diagram(frm) {
	...
	try {
		...
		wrapper.html(
			`<div style="color:red;padding:10px;background:#ffe6e6;border-radius:4px;"><strong>Error:</strong> Invalid JSON data</div>`,
		);
		return;
	}
	...
	if (!data.length) {
		wrapper.html(
			`<div style="color:#888;padding:10px;">No hierarchy defined yet. Add roles above to get started.</div>`,
		);
		return;
	}
	...
	if (emptyParents.length > 0) {
		wrapper.html(
			`<div style="color:red;padding:10px;background:#ffe6e6;border-radius:4px;"><strong>Error:</strong> Found rows with empty parent roles. Please fill in all parent roles.</div>`,
		);
		return;
	}
