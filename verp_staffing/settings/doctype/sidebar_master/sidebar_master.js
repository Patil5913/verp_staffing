...
frappe.ui.form.on("Sidebar Master", {
    ...
    validate(frm) {
		save_canvas_to_json(frm); // ← flush canvas state into config_json before serialization
	},
    after_save(frm) {
		// Clear stale cache + remount sidebar with fresh config — no full page reload
		if (typeof window.csb_reload === "function") {
			window.csb_reload();
		} else {
			// Fallback: hard refresh if override.js hasn't loaded for some reason
...
function inject_styles() {
	...
	if (el) el.remove(); // always refresh so re-opens get latest styles
	const s = document.createElement("style");
	s.id = "sb-styles";
	s.textContent = `
	/* ═══ ROOT ═══════════════════════════════════════════════════ */
	#sb-root { font-size:13px; border:1px solid var(--border-color); border-radius:8px; overflow:hidden; background:var(--bg-color); }
	#sb-root * { box-sizing:border-box; }

	/* ═══ GUIDE BANNER ══════════════════════════════════════════ */
	.sb-guide-banner { display:flex; align-items:flex-start; gap:12px; padding:10px 14px; background:var(--alert-bg,#eff6ff); border-bottom:1px solid var(--border-color); font-size:11px; color:var(--text-color); line-height:1.5; }
	...
	.sb-guide-step { display:flex; align-items:center; gap:4px; font-size:10px; color:var(--text-muted); background:var(--fg-color); border:1px solid var(--border-color); border-radius:20px; padding:2px 8px; }
	...
	.sb-palette-head-top .label { font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.07em; color:var(--text-muted); }
	.sb-palette-head-top .count { font-size:10px; color:var(--text-muted); }
	.sb-search { position:relative; }
	.sb-search input { width:100%; padding:5px 8px 5px 26px; border:1px solid var(--border-color); border-radius:5px; font-size:11px; background:var(--control-bg); color:var(--text-color); outline:none; }
	.sb-search input:focus { border-color:var(--primary); }
	.sb-search .search-icon { position:absolute; left:7px; top:50%; transform:translateY(-50%); color:var(--text-muted); font-size:11px; }
	.sb-filter-tabs { display:flex; gap:3px; }
	.sb-tab { padding:2px 8px; border-radius:20px; font-size:10px; font-weight:600; cursor:pointer; border:1px solid var(--border-color); color:var(--text-muted); background:transparent; }
	...
	.sb-group-label { padding:7px 10px 2px; font-size:9px; font-weight:700; text-transform:uppercase; letter-spacing:.08em; color:var(--text-muted); }
	...
	.sb-palette-item:not(.in-use)::after { content:"drag →"; font-size:9px; color:var(--text-muted); opacity:0; margin-left:auto; text-transform:uppercase; letter-spacing:.04em; flex-shrink:0; }
	...
	.pi-name { font-size:11px; font-weight:500; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; color:var(--text-color); }
	.pi-type { font-size:9px; color:var(--text-muted); text-transform:uppercase; letter-spacing:.04em; }
	...
	.sb-canvas-head .label { font-size:10px; font-weight:700; text-transform:uppercase; letter-spacing:.07em; color:var(--text-muted); }
	.sb-canvas-head-right { display:flex; align-items:center; gap:10px; }
	.sb-canvas-tip { font-size:10px; color:var(--text-muted); }

	/* Canvas body — the scrollable drop area */
	.sb-canvas-body { flex:1; overflow-y:auto; padding:10px 10px 40px; }
	.sb-empty-state { text-align:center; padding:50px 20px; color:var(--text-muted); font-size:13px; }
	.sb-empty-state i { font-size:36px; margin-bottom:10px; display:block; opacity:.35; }
	.sb-empty-state p { margin:4px 0 0; font-size:11px; }

	/* Drop overlay while a palette item is airborne */
	...
	/* Dim non-child areas while dragging INTO a child zone */
	...
	/* Single-link: no border at bottom (no children zone) */
	.sb-section[data-ptype="type_3"] .sb-section-header { border-radius:4px; }

	.sh-drag { color:var(--text-muted); font-size:14px; cursor:grab; padding:2px; border-radius:3px; flex-shrink:0; line-height:1; }
	.sh-drag:hover { color:var(--primary); background:var(--control-bg); }
	.sh-drag:active { cursor:grabbing; }

	.sh-icon-btn { width:24px; height:24px; border-radius:5px; border:1px dashed var(--border-color); background:var(--bg-color); display:flex; align-items:center; justify-content:center; cursor:pointer; flex-shrink:0; }
	.sh-icon-btn:hover { border-color:var(--primary); background:var(--primary-light,#ebf5fb); }

	.sh-label { flex:1; border:none; background:transparent; font-size:13px; font-weight:600; color:var(--text-color); outline:none; cursor:text; min-width:30px; }
	.sh-label:focus { background:var(--control-bg); border-radius:3px; padding:1px 4px; }

	/* Type badge (compact, replaces 3 buttons when not hovered) */
	...
	.sh-type-dropdown {
		...
		padding:4px; min-width:200px; z-index:9999;
	}
	...
	.sh-type-option-label { font-weight:600; font-size:11px; }
	.sh-type-option-desc { font-size:10px; color:var(--text-muted); margin-top:1px; }
	.sh-type-locked { font-size:10px; color:var(--text-muted); padding:2px 8px; background:var(--bg-color); border:1px solid var(--border-color); border-radius:20px; }

	.sh-del-btn { width:22px; height:22px; border-radius:4px; border:none; background:transparent; color:var(--text-muted); cursor:pointer; font-size:13px; display:flex; align-items:center; justify-content:center; flex-shrink:0; }
	.sh-del-btn:hover { background:var(--red-100,#fde8e8); color:var(--red-500,#e53e3e); }

	/* ── Shortcut pill (inline in header) ── */
	.sh-shortcut-pill {
	...
	.sh-shortcut-pill .sp-icon { font-size:11px; }
	...
	.sb-sp-title { font-size:13px; font-weight:700; color:var(--text-color); margin-bottom:4px; }
	.sb-sp-sub   { font-size:11px; color:var(--text-muted); margin-bottom:14px; line-height:1.5; }
	.sb-sp-keys  {
		display:flex; align-items:center; justify-content:center; gap:6px;
		padding:14px; background:var(--bg-color); border-radius:7px;
		border:2px dashed var(--border-color); margin-bottom:12px;
		min-height:52px; font-size:12px; color:var(--text-muted);
	...
	.sb-sp-key {
		...
		padding:4px 9px; background:var(--fg-color); border:1.5px solid var(--border-color);
		border-radius:5px; box-shadow:0 2px 0 var(--border-color);
		font-size:12px; font-weight:700; font-family:monospace; color:var(--text-color);
		min-width:32px;
	}
	.sb-sp-plus { color:var(--text-muted); font-size:13px; font-weight:700; }
	.sb-sp-prompt { color:var(--text-muted); font-size:11px; }
	.sb-sp-error  { font-size:11px; color:var(--red-500,#e53e3e); font-weight:600; text-align:center; min-height:16px; margin-bottom:8px; }
	.sb-sp-actions { display:flex; align-items:center; justify-content:space-between; gap:8px; }
	.sb-sp-btn {
		padding:5px 14px; border-radius:6px; border:1.5px solid var(--border-color);
		background:var(--bg-color); color:var(--text-muted); font-size:11px;
	...
	.sb-section-divider .sd-count { font-size:9px; color:var(--text-muted); }

	/* ═══ CHILDREN ZONE ══════════════════════════════════════════ */
	.sb-children {
		padding:0 8px 8px 28px; /* <-- left indent creates the sidebar-like visual */
		min-height:52px;
		position:relative;
		background:var(--bg-color);
		border-radius:0 0 6px 6px;
	}
	/* Drop zone hint bar at the top of the children zone */
	.sb-children-drop-bar {
		display:flex; align-items:center; justify-content:center; gap:6px;
		margin-bottom:4px; padding:5px 8px;
		border:1.5px dashed var(--border-color); border-radius:5px;
		font-size:10px; color:var(--text-muted);
		background:transparent; transition:all .15s;
	}
	.sb-children.drop-over .sb-children-drop-bar {
		border-color:var(--primary); background:var(--primary-light,#ebf5fb);
	...
	.sb-child::before {
		content:""; position:absolute;
		left:-14px; top:50%; width:13px; height:1px;
	...
	.ci-drag { color:var(--text-muted); font-size:11px; cursor:grab; flex-shrink:0; padding:1px 2px; border-radius:3px; line-height:1; }
	.ci-drag:hover { color:var(--primary); background:var(--control-bg); }
	.ci-drag:active { cursor:grabbing; }
	.ci-icon-btn { width:16px; height:16px; border-radius:3px; background:var(--control-bg); display:flex; align-items:center; justify-content:center; flex-shrink:0; cursor:pointer; border:1px solid transparent; }
	.ci-icon-btn:hover { border-color:var(--primary); background:var(--primary-light,#ebf5fb); }
	.ci-label { flex:1; font-size:11px; font-weight:500; color:var(--text-color); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
	.ci-badge { font-size:9px; color:var(--text-muted); background:var(--bg-color); border:1px solid var(--border-color); border-radius:3px; padding:1px 5px; text-transform:uppercase; letter-spacing:.04em; flex-shrink:0; }

	/* Child shortcut pill */
	.ci-shortcut {
		display:inline-flex; align-items:center; gap:3px;
		padding:1px 6px; border-radius:10px; border:1px dashed var(--border-color);
	...
	.ci-del { width:16px; height:16px; border-radius:50%; border:none; background:transparent; color:var(--text-muted); cursor:pointer; font-size:12px; display:flex; align-items:center; justify-content:center; opacity:0; flex-shrink:0; }
	...
	.sb-icon-chip { display:flex; flex-direction:column; align-items:center; gap:3px; padding:6px 3px; border-radius:5px; border:1px solid var(--border-color); cursor:pointer; font-size:9px; color:var(--text-muted); text-align:center; background:var(--bg-color); }
...
let sb_sections = [];
let sb_filter = "all";
let sb_drag = null; // { kind:"palette"|"section"|"child", ... }

// Shortcut popup state
let sb_sp_context = null; // { sec, child (optional), refresh_zone (optional), pill }
let sb_sp_pending = ""; // shortcut being recorded
...
// All shortcuts in use, excluding a given owner id (sec._id or child pseudo id)
...
function sb_palette_drag_end() {
	...
	if (b) {
		b.classList.remove("palette-drag-active", "drag-over-canvas", "child-zone-active");
	}
}
...
function add_item_to_canvas(item) {
	// Reports and pages are always direct single links — no children, no dropdown
...
function render_canvas() {
	...
	// Canvas-level drop (palette → new top-level section)
	if (body && !body._sb_canvas_bound) {
		body._sb_canvas_bound = true;
		body.addEventListener("dragover", (e) => {
			if (!sb_drag || sb_drag.kind !== "palette") return;
			if (e.target.closest(".sb-children")) return; // hand off to zone
			e.preventDefault();
		...
		// canvas-level drop (new top-level section)
		body.addEventListener("drop", (e) => {
			...
			sb_drag = null;
			sb_palette_drag_end();
			if (sb_used_keys().has(item.key)) return; // guard
...
function render_section(sec, idx, container) {
	...
	// Shortcut pill
	const sc_label = sec.shortcut ? sec.shortcut : "⌨ shortcut";
	...
	hdr.innerHTML = `
		...
		<input class="sh-label" value="${frappe.utils.escape_html(sec.label)}" placeholder="Label" spellcheck="false">
		${type_html}
	...
	// Label
	hdr.querySelector(".sh-label").addEventListener("input", (e) => {
	...
	if (!is_locked) {
		...
		hdr.querySelectorAll(".sh-type-option").forEach((opt) => {
			opt.addEventListener("click", () => {
				sec.parent_type = opt.dataset.type;
	...
	// ── Children zone (type_1 and type_2 only) ──
	if (!is_type3) {
		// Divider
...
function make_children_zone(sec, sectionEl) {
	...
	const refresh_zone = () => {
		zone.innerHTML = "";
		// Always-visible drop bar at top of zone
	...
	// Drag enter/over — palette drops only; highlight zone + dim rest
	...
	zone.addEventListener("dragover", (e) => {
		...
		if (sb_drag.kind === "palette") {
			...
			e.preventDefault(); // allow bubble to individual child rows
		}
	});
	...
	zone.addEventListener("drop", (e) => {
		...
		sb_drag = null;
		sb_palette_drag_end();
		if (sb_used_keys().has(item.key)) return; // guard — blocks dupes in same parent too
		sec.children.push({
			key: item.key,
			name: item.name,
			type: item.type,
...
function bind_shortcut_popup() {
	...
	// Chord state: track first key of a two-key combination
	// e.g. user holds J then presses E → "J+E"
	let chord_first = null; // first key char if in chord-waiting state
	let chord_timer = null; // timeout to commit single-key if no second key comes

	const CHORD_WAIT_MS = 1000; // how long to wait for a second key after the first

	// Keys that are pure modifiers — ignore them alone
	...
	function render_waiting_for_second(firstKey) {
		keysEl.innerHTML = `
			...
			<span class="sb-sp-prompt" style="font-size:10px">press second key or wait…</span>
		`;
		keysEl.classList.remove("error");
		keysEl.classList.add("recording");
	}
	...
	function handle_key(e) {
		// Always capture to block browser shortcuts while popup is open
		e.preventDefault();
		e.stopPropagation();
		...
		if (MODIFIER_KEYS.has(e.key)) return; // pure modifier, skip

		const raw_key = e.key.length === 1 ? e.key.toUpperCase() : e.key; // "A"–"Z", "0"–"9", or named key
		const has_alt = e.altKey;
		const has_ctrl = e.ctrlKey;
		const has_shift = e.shiftKey && e.key.length > 1; // shift+letter gives uppercase anyway
		...
		if (raw_key.toLowerCase() === "n" && !chord_first) {
			// Block bare "n" only when it would be the first/only key
			show_error(
		...
		if (chord_first) {
			...
			const second = raw_key;
			shortcut = chord_first + "+" + second;
			reset_chord();
			commit_shortcut(shortcut);
		} else {
			// First key pressed — start chord-waiting window
			chord_first = raw_key;
			render_waiting_for_second(chord_first);

			// After CHORD_WAIT_MS with no second key, commit as single-key
	...
	function handle_keyup() {
		// nothing needed — chord logic is timeout-based
	}
	...
	// Store reference so open_shortcut_popup can access inner fns
...
function bind_toolbar(frm) {
	...
	// 	.off("click.sb_flush")
	// 	.on("click.sb_flush", () => save_canvas_to_json(frm));
}
