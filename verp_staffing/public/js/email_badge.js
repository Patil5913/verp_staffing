(function () {
	...
	function injectStyles() {
		...
		s.textContent = [
			...
			"margin-left:auto;" /* pushes badge to the right inside the flex row */,
			"margin-right:2px;",
			"flex-shrink:0;",
			"pointer-events:none;",
			"box-shadow:0 1px 3px rgba(226,75,74,.40)",
			"}",
			...
			/* Ensure our sidebar anchor is a flex row so the badge
         naturally aligns to the right via margin-left:auto.
         .cn-item is already display:flex, but be explicit here. */
			"#custom-nav-sidebar .cn-item{",
	...
	/* ── Paint / refresh badge DOM ── */
	function setBadge(count) {
		_badgeCount = count;

		// Expose globally so other scripts can call window.__ewSetBadge(n)
		window.__ewSetBadge = setBadge;
		window.__ewUnreadCount = count;

		// Remove any existing badges first
		document.querySelectorAll(".ew-sidebar-badge").forEach(function (b) {
			b.remove();
		});

		if (count <= 0) return;

		// Target: the anchor inside our custom sidebar whose parent-key is "email-inbox"
		// Selector: a.cn-item[data-parent-key="email-inbox"]
		// Fallback:  any .cn-item whose .cn-item-label text is "Email Inbox"
		var anchors = document.querySelectorAll(
			'#custom-nav-sidebar a.cn-item[data-parent-key="email-inbox"]',
		);

		// Fallback — label text match (handles key name variations)
		if (!anchors.length) {
			anchors = Array.prototype.filter.call(
				document.querySelectorAll("#custom-nav-sidebar a.cn-item"),
				function (a) {
					var lbl = a.querySelector(".cn-item-label");
					return lbl && lbl.textContent.trim() === "Email Inbox";
				},
			);
		}
	...
	function registerRealtimeHandlers() {
		...
		frappe.realtime.on("new_email", function () {
			...
			window._ewBadgeNewTimer = setTimeout(function () {
				const widgetPushedRecently =
					window.__ewUnreadCountTs && Date.now() - window.__ewUnreadCountTs < 2000;
   ...
   *
   ──────────────────────────────────────────────────────────────────────── */
	function bindPageChange() {
		$(document)
			.off("page-change.ew-badge")
			.on("page-change.ew-badge", function () {
				// 1 — instant repaint with last cached count
				setTimeout(function () {
					setBadge(_badgeCount);
				}, 250);
