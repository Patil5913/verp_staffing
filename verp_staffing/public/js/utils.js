...
verp_staffing.accounts = verp_staffing.accounts || {};
...
verp_staffing.accounts.taxes = {
			...
			onload: function (frm) {
				if (frm.get_field("taxes")) {
					frm.set_query("account_head", "taxes", function (doc) {
						...
						return {
							query: "verp_staffing.utils.accounts.tax_account_query",
							filters: {
		...
		frappe.ui.form.on(doctype, {
			account_head: function (frm, cdt, cdn) {
				...
				if (!d.charge_type && d.account_head) {
					...
					frappe.call({
						type: "GET",
						method: "verp_staffing.utils.accounts.get_tax_rate",
			...
			charge_type: function (frm, cdt, cdn) {
				...
				if (open_form) {
					...
					me.set_conditional_mandatory_rate_or_amount(
						frm.get_field("taxes").grid.get_row(cdn),
					);
				}
				me.recalculate_taxes(frm);
			},
	...
	recalculate_taxes: function (frm) {
		frappe.call({
			method: "verp_staffing.utils.taxes.calculate_taxes_and_totals_function",
			args: {
				doc: frm.doc,
			},
			callback: function (r) {
				...
				// 🔥 update child tables explicitly
				frm.set_value("items", d.items);
				frm.set_value("taxes", d.taxes);

				// 🔥 update totals
				frm.set_value("total", d.total);
				frm.set_value("net_total", d.net_total);
				frm.set_value("grand_total", d.grand_total);
				frm.set_value("base_total", d.base_total);
				frm.set_value("base_net_total", d.base_net_total);
				frm.set_value("base_grand_total", d.base_grand_total);
				frm.set_value("total_taxes_and_charges", d.total_taxes_and_charges);
