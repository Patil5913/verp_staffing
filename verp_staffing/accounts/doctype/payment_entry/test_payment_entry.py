    ...
    **overrides,
    ...
    if not party:
        if party_type == "Customer":
            party = make_customer(f"_Test Customer {company}")
        else:
    ...
    if references:
        total_allocated = sum(flt(r.get("allocated_amount", 0)) for r in references)
...
class TestPaymentEntry(PaymentEntry):
    ...
    def test_zero_paid_amount_is_rejected(self):
        ...
        customer = make_customer(f"_Test Customer {self.company}")
        ...
        pe.party = customer
        pe.paid_from = get_default_company_account(self.company, "Receivable")
