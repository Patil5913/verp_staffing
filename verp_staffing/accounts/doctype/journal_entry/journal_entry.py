...
# Copyright (c) 2026, Vrugle
# Custom lightweight Journal Entry for verp_staffing
...
class JournalEntry(Document):
    ...
    def validate(self):
        ...
        self.validate_same_account_debit_credit()

    # -----------------------------
    # 🔹 SUBMIT → GL ENTRY
    # -----------------------------
    def on_submit(self):
        # delete old GL (safe re-submit)
    ...
    def validate_same_account_debit_credit(self):
        for row in self.accounts:
            if flt(row.debit) and flt(row.credit):
                frappe.throw(_(f"Row {row.idx}: Cannot debit and credit same account"))

    def validate_balance(self):
        if round(self.difference, 2) != 0:
            frappe.throw(_(f"Total Debit must equal Total Credit. Difference: {self.difference}"))
