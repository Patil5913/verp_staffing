...
class GLEntry(Document):
    ...
    def validate_party(self):
        ...
        if account_type in ["Receivable", "Payable"]:
            if not self.party:
                frappe.throw(
                    _("Party is required for Receivable/Payable account {0}").format(
                        self.account
                    )
                )
    ...
    def validate_currency(self):
        ...
        if self.transaction_currency and self.transaction_currency != company_currency:
            if not self.exchange_rate:
                frappe.throw(
                    _(
                        "Exchange Rate is required when transaction currency is different from company currency"
                    )
                )
...
def cancel_gl_entries(doc, method=None):
    if frappe.db.exists(
        "GL Entry",
        {"voucher_type": doc.doctype, "voucher_no": doc.name, "is_cancelled": 1},
    ):
        frappe.throw(
            "GL Entries already cancelled for {0} {1}".format(doc.doctype, doc.name)
        )
    ...
    for e in entries:
        ...
        new_debit = flt(original.credit)
        new_credit = flt(original.debit)

        if (new_debit and new_credit) or (not new_debit and not new_credit):
            continue
        ...
        reverse = build_gl_entry(
            ...
            against_voucher_type=original.against_voucher_type,
        ...
        frappe.get_doc({"doctype": "GL Entry", **reverse, "is_cancelled": 1}).insert(
...
def get_fiscal_year(posting_date, company=None):
    ...
    if company:
        valid_fy = frappe.get_all(
            "Fiscal Year Company",
            filters={"parent": ["in", fy_names], "company": company},
            pluck="parent",
            limit=1,
        )
...
def enrich_gl_entry(entry, doc):
    ...
    exchange_rate = (
        flt(entry.get("exchange_rate")) or flt(getattr(doc, "conversion_rate", 0)) or 1
    )

    # Transaction currency
    entry["transaction_currency"] = entry.get("transaction_currency") or getattr(
        doc, "currency", None
    )
...
def merge_gl_entries(gl_map):
    ...
    for entry in grouped.values():
        ...
        if debit > credit:
            entry["debit"] = round(debit - credit, 9)
            entry["credit"] = 0
        elif credit > debit:
            entry["credit"] = round(credit - debit, 9)
            entry["debit"] = 0
        else:
            continue
