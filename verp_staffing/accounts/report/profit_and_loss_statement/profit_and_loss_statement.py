...
def _get_fiscal_years_in_range(from_fiscal_year, to_fiscal_year):
    """
    Return an ordered list of all fiscal year names between
    from_fiscal_year and to_fiscal_year (inclusive), sorted by
    year_start_date so the range is date-order safe.
    """
    rows = frappe.db.sql(
        """
        SELECT fy.name
        FROM `tabFiscal Year` fy
        WHERE fy.year_start_date BETWEEN
            (SELECT year_start_date FROM `tabFiscal Year` WHERE name = %s)
            AND
            (SELECT year_end_date   FROM `tabFiscal Year` WHERE name = %s)
        ORDER BY fy.year_start_date ASC
    """,
...
def get_data(filters):
    ...
    # ── Resolve fiscal years for GL filtering ─────────────────────
    if filters.filter_based_on == "Fiscal Year":
        all_fiscal_years = _get_fiscal_years_in_range(
            filters.from_fiscal_year, filters.to_fiscal_year
        )
    else:
        all_fiscal_years = None
    ...
    for label, p_from, p_to in period_list:
        ...
        if all_fiscal_years and accumulated and len(all_fiscal_years) > 1:
            fy_for_period = (
                _get_fiscal_year_for_date(p_to, all_fiscal_years) or all_fiscal_years
            )
        else:
            fy_for_period = all_fiscal_years

        gl = get_gl_balances(
            filters, query_from, p_to, all_account_names, fiscal_years=fy_for_period
        )
    ...
    for root in [
        a.name for a in accounts if a.root_type == "Income" and not a.parent_account
    ]:
    ...
    for root in [
        a.name for a in accounts if a.root_type == "Expense" and not a.parent_account
    ]:
        add_rows(root, indent=1)
    ...
    exchange_rate = flt(filters.get("presentation_exchange_rate") or 1)
    if exchange_rate and exchange_rate != 1.0:
        for row in data:
            if not row:  # spacer rows
                continue
            for label, _pf, _pt in period_list:
                fn = frappe.scrub(label)
                if fn in row:
                    row[fn] = flt(row[fn]) * exchange_rate
...
def _get_fiscal_year_for_date(check_date, fy_names):
    ...
    date range contains check_date.  Returns the full list if
    none match (safe fallback).
    """
    if not fy_names:
        return fy_names
    rows = frappe.db.sql(
