...
frappe.query_reports["Profit and Loss Statement"] = {
	...
	filters: [
		...
		// ── Row 1 ──────────────────────────────────────────────────
		{
			...
			on_change: function () {
				...
				if (company) {
					frappe.query_reports["Profit and Loss Statement"]
						._apply_company_defaults(company, true);
				}
			}
		},
		{
		...
		// ── Checkboxes ─────────────────────────────────────────────
		{
			...
			fieldtype:   "Check",
			default:     0,
			description: __("When checked, each period column shows year-to-date totals instead of that period's movement.")
		},
		{
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (view === "Margin View" && column.fieldname !== "account") {
			...
			let color = pct < 0 ? "var(--red-500)" : (pct > 0 ? "var(--green-500)" : "var(--text-muted)");
		...
		if (view === "Growth View" && column.fieldname !== "account"
		...
		if (data.is_net_profit && column.fieldname !== "account") {
		...
		// Any other negative amounts in red
		if (!data.is_net_profit && typeof value === "number" && value < 0) {
